...
describe('design-decisions.md template validation', () => {
  const templatePath = path.join(__dirname, '../../src/resources/templates/specs/design-decisions.md');
  let content: string;
  ...
  it('should use H1 format with feature name placeholder', () => {
    expect(content).toMatch(/^# Design Decisions: \{feature-name\}$/m);
  });
...
describe('design-decisions.md instance validation', () => {
  ...
  const getDecisionFiles = () => {
    const features = fs.readdirSync(featuresDir, { withFileTypes: true })
      ...
      .map(d => d.name);
    return features
      .map(f => ({ feature: f, path: path.join(featuresDir, f, 'design-decisions.md') }))
  ...
  it('should not contain authority-based reasoning', () => {
    for (const { path: p } of getDecisionFiles()) {
      ...
      for (const why of whyBlocks) {
        expect(why).not.toMatch(/per user (request|specification|requirement)/i);
      }
    }
  });
});
