...
describe('AI Commands', () => {
  ...
  describe('AIProviderCommandConfig', () => {
    const providers: ProviderCommandEntry[] = Object.values(PROVIDER_COMMAND_CONFIGS);

    // @req FR:ai-provider/provider.command-config
    it('should define required fields on all provider command configs', () => {
      for (const provider of providers) {
        const { commands } = provider;
        expect(commands).toEqual(
          expect.objectContaining({
            path: expect.any(String),
            useNamespaces: expect.any(Boolean),
            separator: expect.any(String),
            useFrontMatter: expect.any(Boolean),
  ...
  describe('generateProviderCommands', () => {
    ...
    it('should handle unreadable template files gracefully', () => {
      ...
      // Should still generate for good.md (3 providers) + 3 .gitignore files
    ...
    it('should filter out non-.md files from templates', () => {
      ...
      // Only rollout.md should be processed (3 providers x 1 file = 3 writes) + 3 .gitignore files
      expect(mockFs.writeFileSync).toHaveBeenCalledTimes(6);
    });
    ...
    it('should generate files for all 3 providers per template', () => {
      ...
      // 3 providers x 1 template = 3 files + 3 .gitignore files
      expect(mockFs.writeFileSync).toHaveBeenCalledTimes(6);
    });
  ...
  describe('writeCommandGitignore', () => {
    const projectRoot = '/test/project';
    const claudeProvider: ProviderCommandEntry = PROVIDER_COMMAND_CONFIGS.claude;
    const copilotProvider: ProviderCommandEntry = PROVIDER_COMMAND_CONFIGS.copilot;
    const cursorProvider: ProviderCommandEntry = PROVIDER_COMMAND_CONFIGS.cursor;
    ...
    it('should write .gitignore with * for folder-based providers (Claude)', () => {
      writeCommandGitignore(projectRoot, claudeProvider);

      expect(mockFs.writeFileSync).toHaveBeenCalledWith(
        path.join(projectRoot, '.claude/commands/catalyst/.gitignore'),
    ...
    // @req FR:ai-provider/gitignore.folder
    // @req FR:ai-provider/gitignore.header
    it('should write .gitignore with * for folder-based providers (Cursor)', () => {
      writeCommandGitignore(projectRoot, cursorProvider);

      expect(mockFs.writeFileSync).toHaveBeenCalledWith(
        path.join(projectRoot, '.cursor/commands/catalyst/.gitignore'),
        '# Generated by Catalyst — do not edit manually\n*\n'
      );
    });
    ...
    // @req FR:ai-provider/gitignore.header
    it('should merge catalyst pattern into existing .gitignore for flat providers', () => {
      mockFs.readFileSync.mockReturnValue('# Existing rules\nother-pattern\n');

      writeCommandGitignore(projectRoot, copilotProvider);

      expect(mockFs.writeFileSync).toHaveBeenCalledWith(
        path.join(projectRoot, '.github/prompts/.gitignore'),
        '# Existing rules\nother-pattern\n\n# Generated by Catalyst — do not edit manually\ncatalyst.*.prompt.md\n'
      );
    });
    ...
    // @req FR:ai-provider/gitignore.header
    it('should create new .gitignore with catalyst pattern for flat providers when none exists', () => {
    ...
    it('should not duplicate catalyst pattern if already present in flat provider .gitignore', () => {
      mockFs.readFileSync.mockReturnValue('catalyst.*.prompt.md\nother-rule\n');

      writeCommandGitignore(projectRoot, copilotProvider);

      // Should not write at all since pattern already exists
      expect(mockFs.writeFileSync).not.toHaveBeenCalled();
    });

    // @req FR:ai-provider/gitignore.folder
    it('should create target directory for folder-based providers', () => {
      writeCommandGitignore(projectRoot, claudeProvider);
