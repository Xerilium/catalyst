...
3. **Map and validate inputs**
   - Map command inputs to playbook inputs:
     ...
     - `ai-platform` → "$$AI_PLATFORM$$" (automatically set based on the AI platform running this command)
...
- [ ] All responses use the `[Catalyst][$$AI_PLATFORM$$]` prefix
...
2. Identify threads needing replies (excluding those already replied to by `[Catalyst][$$AI_PLATFORM$$]`)
...
6. Post summary comment on PR with `[Catalyst][$$AI_PLATFORM$$]` prefix
