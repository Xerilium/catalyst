...
**Next**: Run 4 Phase 0 (Scope). Checkpoint-review mode. Add `gitignore-write` action to `playbook-actions-io`; refactor install-ai-providers.yaml gitignore script blocks.
...
## Run 4: gitignore-write action

> [INSTRUCTIONS]
> Detailed task breakdown deferred to Run 4's Phase 0. High-level:

- Add `gitignore-write` to `playbook-actions-io` (idempotent merge of Catalyst-marked section into a .gitignore file)
- Refactor install-ai-provider.yaml to use `gitignore-write` instead of `script`-based merge
- Optional: refactor feature-context's `FR:index.gitignore` to also use the new action (boy scout if low-risk)
- No ai-provider or cli-init spec change
