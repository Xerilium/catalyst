...
**FR-3A**: Action Dependency Metadata

- **FR-3A.1**: System MUST define `PlaybookActionDependencies` interface with the following properties:
...
- **FR-3A.2**: System MUST define `CliDependency` interface with the following properties:
...
- **FR-3A.3**: System MUST define `EnvDependency` interface with the following properties:
...
- **FR-3A.4**: System MUST define `CheckResult` interface for dependency validation results:
...
- **FR-3A.5**: System MUST provide `DependencyChecker` service with the following methods:
...
- **FR-3A.6**: System MUST provide auto-generated dependency registry at build time:
  - Registry maps action types (kebab-case) to PlaybookActionDependencies
  - Generated by scanning all *-action.ts files for static dependencies property
  - Exported as `DEPENDENCY_REGISTRY` constant from registry/dependency-registry.ts
  - Build script MUST fail if scanning encounters errors
...
- **DEPENDENCY_REGISTRY**: Auto-generated registry mapping
  - Key: Action type (kebab-case, e.g., 'bash-run')
  - Value: PlaybookActionDependencies metadata
  - Generated at build time by scanning action files
