...
**FR:types.playbook**: TypeScript Playbook Interfaces

- **FR:types.playbook.interface**: System MUST define `Playbook` interface with the following properties:
...
- **FR:validation.base**: System MUST define `ValidationRule` base interface with the following properties:
...
- **FR:validation.regex**: System MUST define `RegexValidationRule` interface extending ValidationRule:
  - `type` (string, required): Must be 'Regex'
  - `pattern` (string, required): Regular expression pattern to match

- **FR:validation.string-length**: System MUST define `StringLengthValidationRule` interface extending ValidationRule:
...
- **FR:validation.number-range**: System MUST define `NumberRangeValidationRule` interface extending ValidationRule:
...
- **FR:validation.custom**: System MUST define `CustomValidationRule` interface extending ValidationRule:
  - `type` (string, required): Must be 'Custom'
  - `script` (string, required): JavaScript expression that returns boolean (true = valid)

- **FR:validation.union**: System MUST define `InputValidationRule` as union type of all validation rule interfaces:
...
- **FR:validation.validator**: System MUST provide `Validator` interface for extensible validation:
...
- **FR:validation.factory**: System MUST provide `ValidatorFactory` class for validator registration:
...
- **FR:validation.result**: System MUST define `ValidationResult` interface with the following properties:
  - `valid` (boolean, required): Whether validation passed
  - `error` (ValidationError, optional): Error details if validation failed

- **FR:validation.error**: System MUST define `ValidationError` interface with the following properties:
...
**FR:types.step**: TypeScript Step Interface

- **FR:types.step.interface**: System MUST define `PlaybookStep` interface with the following properties:
...
- **FR:types.step.unique-names**: Step names MUST be unique within a playbook when specified

- **FR:types.step.config**: Config property contains action-specific configuration object
...
- **FR:types.action.interface**: System MUST define `PlaybookAction<TConfig>` interface with the following:
...
- **FR:types.action.result**: System MUST define `PlaybookActionResult` interface with the following properties:
...
- **FR:types.action.dependencies**: System MUST support optional action dependency metadata via `dependencies` property on `PlaybookAction` interface
...
- **FR:types.step-executor.variable-scoping**: Variable overrides MUST be additive and scoped
...
- **FR:types.step-executor.execution-rules**: StepExecutor implementation MUST enforce all execution rules
...
- **FR:types.dependencies.interface**: System MUST define `PlaybookActionDependencies` interface with the following properties:
...
- **FR:types.dependencies.cli**: System MUST define `CliDependency` interface with the following properties:
...
- **FR:types.dependencies.env**: System MUST define `EnvDependency` interface with the following properties:
...
- **FR:types.dependencies.check-result**: System MUST define `CheckResult` interface for dependency validation results:
...
- **FR:types.dependencies.checker**: System MUST provide `DependencyChecker` service with the following methods:
...
- **FR:catalog.generation**: System MUST generate action catalog at build time by scanning action implementations
...
- **FR:catalog.metadata**: System MUST define `ActionMetadata` interface with the following properties:
...
- **FR:catalog.extract-classname**: System MUST extract `className` from action class export name
...
- **FR:catalog.extract-actiontype**: System MUST extract `actionType` from action class static readonly property
...
- **FR:catalog.extract-primaryproperty**: System MUST extract `primaryProperty` from action class static property
...
- **FR:catalog.generate-schema**: System MUST generate `configSchema` from TypeScript config interfaces using `typescript-json-schema`
...
- **FR:catalog.internal**: Generated catalog MUST be internal to PlaybookProvider
...
- **FR:catalog.build-integration**: Catalog generation MUST run as part of build process before TypeScript compilation
...
- **FR:schema.typescript-json-schema**: System MUST use `typescript-json-schema` library to generate JSON schemas from config interfaces
...
- **FR:schema.jsdoc-preservation**: Generated schemas MUST preserve JSDoc comments as schema descriptions
  - Property descriptions from `/** Property description */` comments
  - Interface description from class-level JSDoc

- **FR:schema.required-properties**: Generated schemas MUST correctly handle optional vs required properties
  - Optional properties (marked with `?`) excluded from `required` array
  - Required properties included in `required` array

- **FR:schema.complex-types**: Generated schemas MUST support complex types:
...
- **FR:types.state.interface**: System MUST define `PlaybookState` interface with the following properties:
...
- **FR:types.state.variables**: Variables map MUST include inputs, var assignments, and step outputs
...
- **FR:persistence.active-runs**: State snapshots MUST be saved to `.xe/runs/run-{runId}.json`
  - Active runs stay in `.xe/runs/` until completion
  - File format: pretty-printed JSON (human-readable)

- **FR:persistence.archive**: Completed run snapshots MUST be archived to `.xe/runs/history/{YYYY}/{MM}/{DD}/run-{runId}.json`
  - Archive occurs when status becomes 'completed' or 'failed'

- **FR:persistence.atomic-writes**: State files MUST use atomic writes to prevent corruption
  - Write to temp file, then rename (atomic operation on most filesystems)

- **FR:persistence.gitignore**: System MUST create `.xe/runs/history/.gitignore` on first history archive
...
- **FR:persistence.class.methods**: System MUST provide `StatePersistence` class with the following methods:
...
- **FR:persistence.class.performance**: State save operations MUST complete within 100ms for states <1MB
- **FR:persistence.class.format**: State files MUST be human-readable JSON (pretty-printed)
- **FR:persistence.class.error-handling**: Corrupted state files MUST throw clear error with recovery instructions

**FR:provider**: Unified Playbook Provider

- **FR:provider.loader-interface**: System MUST define `PlaybookLoader` interface with the following methods:
...
- **FR:provider.class**: System MUST provide `PlaybookProvider` class as unified provider for playbooks AND actions:
...
- **FR:provider.loader-order**: Provider MUST check loaders in registration order during loadPlaybook()
...
- **FR:provider.duplicate-prevention**: Provider MUST prevent duplicate loader names
...
- **FR:provider.action-initialization**: Provider MUST initialize actions from generated catalog on first access
...
- **FR:provider.action-instantiation**: Provider MUST instantiate actions with appropriate dependencies
...
- **FR:provider.caching**: Provider MUST cache loaded playbooks to avoid redundant loader lookups
...
- **FR:provider.dependency-injection**: Provider MUST support dependency injection for testing
...
- **NFR:performance.serialization**: State serialization MUST complete in <100ms for states <1MB
- **NFR:performance.zero-overhead**: Interface definitions MUST have zero runtime overhead (compile away)

**NFR:reliability**: Reliability

- **NFR:reliability.atomic-writes**: State writes MUST be atomic to prevent corruption on crash
- **NFR:reliability.corruption-recovery**: State files MUST be recoverable if corrupted

**NFR:testability**: Testability

- **NFR:testability.mockable**: All interfaces MUST be mockable for unit testing
- **NFR:testability.abstraction**: State persistence MUST be abstracted for test doubles
- **NFR:testability.coverage**: 100% coverage for state persistence operations

**NFR:maintainability**: Maintainability

- **NFR:maintainability.versioning**: Interface changes MUST be versioned
- **NFR:maintainability.breaking-changes**: Breaking changes to interfaces MUST be clearly documented
- **NFR:maintainability.backward-compatibility**: State format changes MUST support backward compatibility for resume
