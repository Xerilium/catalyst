...
- **FR:commands.generate** (P2): System MUST provide `generateProviderCommands(projectRoot: string): void` function
  ...
  - Writes `.gitignore` files per `FR:gitignore` requirements

- **FR:commands.migrate** (P2): Postinstall MUST untrack previously committed generated command files
  - Detects tracked command files via `git ls-files`
  - Runs `git rm --cached` targeting only known generated paths
  - Folder-based: `{commands.path}/catalyst/*`
  - Flat: `{commands.path}/catalyst.*.{extension}`
  - Idempotent — skips when no tracked files are detected

### FR:gitignore: Generated Command Gitignore

Generated command files are regenerated on every `npm install` and MUST NOT be committed to version control. The system writes `.gitignore` files alongside generated commands to prevent accidental commits.

- **FR:gitignore.folder** (P2): Folder-based providers (`useNamespaces: true`) MUST write a `.gitignore` containing `*` in the namespace subdirectory (e.g., `.claude/commands/catalyst/.gitignore`)
  - All generated files including the `.gitignore` itself are ignored
  - File is regenerated by postinstall on every install

- **FR:gitignore.flat-new** (P2): Flat providers (`useNamespaces: false`) MUST create a new `.gitignore` in the provider directory when none exists
  - MUST include `# Generated by Catalyst — do not edit manually` header
  - MUST include `.gitignore` entry so the file itself is ignored
  - MUST include the Catalyst file pattern (e.g., `catalyst.*.prompt.md`)

- **FR:gitignore.flat-merge** (P2): Flat providers MUST merge the Catalyst pattern into an existing `.gitignore`
  - MUST preserve existing non-Catalyst content
  - MUST include `# Generated by Catalyst — do not edit manually` section header
  - MUST separate the Catalyst section with a blank line before it
  - MUST NOT duplicate the pattern if already present

- **FR:gitignore.header** (P2): All generated `.gitignore` content MUST include a `# Generated by Catalyst — do not edit manually` comment header

- **FR:gitignore.idempotent** (P2): Writing `.gitignore` files MUST be idempotent — repeated runs produce the same result without duplicating content
