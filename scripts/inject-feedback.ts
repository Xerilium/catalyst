...
/**
 * Provider command configuration entry — matches the shape generated by
 * scripts/generate-command-configs.ts and consumed by the install playbook.
 */
...
/**
 * Load provider command configurations from the build-generated JSON.
 *
 * Generated by scripts/generate-command-configs.ts at build time from
 * provider class instances. inject-feedback runs AFTER the generator in
 * the build pipeline, so the file is always present when this is called.
 */
function loadProviderConfigs(): Record<string, ProviderCommandEntry> {
  const configsPath = path.join('dist', 'ai-providers', 'command-configs.json');
...
export function injectFeedback(
  projectRoot: string,
  providers?: ProviderCommandEntry[]
): void {
  const resolvedProviders = providers ?? Object.values(loadProviderConfigs());
