...
3. **Map and validate inputs**
   - Map command inputs to playbook inputs:
     ...
     - `ai-platform` → "Copilot" (automatically set based on the AI platform running this command)
...
- [ ] All responses use the `[Catalyst][Copilot]` prefix
...
2. Identify threads needing replies (excluding those already replied to by `[Catalyst][Copilot]`)
...
6. Post summary comment on PR with `[Catalyst][Copilot]` prefix